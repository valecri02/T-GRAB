...
do
    ...
    do
        for CFG in "4 4" "8 8" "16 8"
        do
            set -- $CFG
            LAG=$1
            NUM_WRITE_STEPS=$2
            DATA="($LAG, $NUM_WRITE_STEPS)/associative_recall_sparse-${NUM_SAMPLES}ns-${NUM_NODES}nn-${ACTIVE_NODES}an-${PAIRS_PER_STEP}pps-${QUERY_RATIO}qr-${VAL_RATIO}vr-${TEST_RATIO}tr"
