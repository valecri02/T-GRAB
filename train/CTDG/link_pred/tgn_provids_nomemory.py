...
from ...CTDG.tgn_provids_nomemory import TGNTrainer
...
class LinkPredTGNTrainer(LinkPredTrainer, TGNTrainer):
    ...
    def forward_backbone(
        ...
        batch_src_node_embeddings = z[self.assoc[batch_src]]
        batch_dst_node_embeddings = z[self.assoc[batch_dst]]
        if batch_neg_dst is None:
            batch_neg_src_node_embeddings = None
            batch_neg_dst_node_embeddings = None
        else:
            batch_neg_src = batch_neg[0]
            batch_neg_src_node_embeddings = z[self.assoc[batch_neg_src]]
            batch_neg_dst_node_embeddings = z[self.assoc[batch_neg_dst]]

        if update_memory:
            self.model['memory'].update_state(
                batch_src,
                batch_dst,
                batch_t,
                batch_edge_feat.float(),
            )
            self.neighbor_loader.insert(batch_src, batch_dst)
