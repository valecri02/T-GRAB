...
from ...model.TGB import LastNeighborLoader
from ...model.TGB.no_memory import TGNNoMemory
from ...model.TGB.provids import GraphAttentionEmbeddingProvIDS, compute_provids_delta_t_stats
...
class TGNTrainer(CTDGTrainer):
    ...
    def get_model(self) -> Dict[str, Module]:
        ...
        self.full_dataset = ContinuousTimeLinkPredNodeFeatureStaticDataset(
        ...
        train_dataset_for_stats = ContinuousTimeLinkPredNodeFeatureStaticDataset(
            os.path.join(self.args.root_load_save_dir, self.args.data_loc),
            self.args.data,
            "train",
            self.args.node_feat,
        ...
        mean_delta_t, std_delta_t = compute_provids_delta_t_stats(
        ...
        memory = TGNNoMemory(
            num_nodes=self.full_dataset.num_nodes,
            memory_dim=node_feat_dim,
            time_dim=self.args.time_feat_dim,
            node_raw_features=self.full_dataset._node_feat.to(self.device),
            init_time=init_time,
        ).to(self.device)
    ...
    def _get_run_save_dir(self) -> str:
        ...
        return os.path.join(ctdgtrainer_run_save_dir, "tgn_provids_nomemory")
