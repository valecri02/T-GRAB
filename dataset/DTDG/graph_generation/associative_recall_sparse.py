...
class AssociativeRecallSparse(GraphGenerator):
    """Sparse delayed key-value recall.

    Each episode writes key-value associations for ``num_write_steps`` only,
    then has an uninformative delay of length ``lag`` before a memory-node query
    and a memory-node target. This recreates the earlier zero-noise setting
    where models must retain sparse bindings without continuous reinforcing
    interactions during the delay.
    """
    ...
    def __init__(self, args) -> None:
        super(AssociativeRecallSparse, self).__init__(
            args.num_nodes,
            args.dataset_name,
            args.seed,
        )
        ...
        match = re.fullmatch(AssociativeRecallSparse._pattern, lag_and_write_steps)
        if not match:
        ...
        if self.lag < 0:
            raise ValueError("associative_recall_sparse requires lag >= 0.")
        if self.num_write_steps <= 0:
            raise ValueError("associative_recall_sparse requires num_write_steps > 0.")
        ...
        self.dataset_name = (
            self.dataset_name
            + f"/associative_recall_sparse-{args.num_samples}ns-{args.num_nodes}nn-"
    ...
    def get_links(self, args_dict) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        with tqdm(total=self.T, desc=self.dataset_name) as pbar:
            for sample_idx in range(self.args.num_samples):
                ...
                for delay_idx in range(self.lag):
                    now_t = cycle_start_t + self.num_write_steps + delay_idx
                    G = nx.empty_graph(self.num_nodes)
                    src, dst, t, edge_feat = self._append_graph(G, now_t, src, dst, t, edge_feat)
                    pbar.update(1)
                ...
                target_values = sorted(set().union(*(bindings[key] for key in query_keys)))
                G = nx.empty_graph(self.num_nodes)
                ...
                if self.args.visualize and sample_idx < 4:
                    ...
                    plt.title(f"Sparse associative recall target timestep {target_t}")
